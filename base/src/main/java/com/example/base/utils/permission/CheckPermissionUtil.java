...
import static com.example.base.utils.ContextUtil.getContext;
...
public class CheckPermissionUtil {
    ...
    public static boolean isLocationPermGrantedAndOpen(){
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ALogUtils.i(TAG,"isLocationPermGrantedAndOpen()---  Build.VERSION.SDK_INT >= Build.VERSION_CODES.M ");
            if (ContextCompat.checkSelfPermission(getContext(), Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED ||
                    ContextCompat.checkSelfPermission(getContext(), Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED )
            {
                ALogUtils.i(TAG,"isLocationPermGrantedAndOpen()---  result = false");
                result = false;
            } else {
                if(isOPenGPS(getContext())){
                    ALogUtils.i(TAG,"isLocationPermGrantedAndOpen()---  result = true");
                    result = true;
                }
            }
        } else {
    ...
    public static boolean isCameraGranted(){
        boolean result = false;
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ALogUtils.i(TAG,"isCameraGranted()---  Build.VERSION.SDK_INT >= Build.VERSION_CODES.M ");
            ...
            {
                ALogUtils.i(TAG,"isCameraGranted()---  result = false");
                result = false;
            } else {
                result = true;
            }
    ...
    public static boolean isContactsPermissionGranted() {
        ...
        ALogUtils.i(TAG, "isContactsPermissionGranted()---  result = " + result);
        return result;
    }
    ...
    public static boolean isOPenGPS(final Context context) {
        ...
        ALogUtils.i("isOPenGPS()","gps = "+gps);
        // 通过WLAN或移动网络(3G/2G)确定的位置（也称作AGPS，辅助GPS定位。主要用于在室内或遮盖物（建筑群或茂密的深林等）密集的地方定位）
        boolean network = locationManager.isProviderEnabled(LocationManager.NETWORK_PROVIDER);
        ALogUtils.i("isOPenGPS()","network = "+network);
        if (gps || network) {
            return true;
        }
        return false;

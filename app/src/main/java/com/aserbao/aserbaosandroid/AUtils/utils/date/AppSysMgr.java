package com.aserbao.aserbaosandroid.AUtils.utils.date;
...
public class AppSysMgr {
	...
	public static String getAppSysMgrInfo(Context context){
        StringBuffer sb = new StringBuffer();
        sb.append("客户端操作系统名称").append(getSysClientOs()).append("\n")
            .append("当前操作系统的sdk版本").append(getSysSdk()).append("\n")
            .append("当前操作系统的语言").append(getSysLanguage()).append("\n")
            .append("当前操作系统的语言").append(getLanguage()).append("\n")
            .append("国家").append(getCountry(context)).append("\n")
            .append("手机型号").append(getSysModel()).append("\n")
            .append("系统版本").append(getOSVersion()).append("\n")
            .append("操作系统的版本号").append(getSysRelease()).append("\n")
            .append("SIM卡序列号").append(getSysSIMSerialNum(context)).append("\n")
            .append("手机CPU序列号").append(getSysCPUSerialNum()).append("\n")
            .append("设备序列号").append(getSerial()).append("\n")
            .append("SIM序列号").append(getSIMSerial(context)).append("\n")
            .append("硬件型号").append(getModel()).append("\n")
            .append("运营商信息(三大运营商)").append(getSysCarrier(context)).append("\n")
            .append("运营商").append(getCarrier(context)).append("\n")
            .append("唯一的设备ID").append(getSysTelephoneSerialNum(context)).append("\n")
            .append("Android设备物理唯一标识符").append(getPsuedoUniqueID()).append("\n")
            .append("手机方位").append(getSysPhoneLoaction(context)).append("\n")
            .append("设备的软件版本号").append(getSysDeviceSoftVersion(context)).append("\n")
            .append("SIM卡提供的移动国家码和移动网络码").append(getSysSimCode(context)).append("\n")
            .append("手机IP地址").append(getSysLocalIpAddress()).append("\n")
            .append("AndroidID").append(getAndroidID(context)).append("\n")
            .append("设备IMSI码").append(getIMSI(context)).append("\n")
            .append("网络IP地址(优先获取wifi地址)").append(getIP(context)).append("\n")
            .append("获取WIFI连接下的ip地址").append(getWifiIP(context)).append("\n")
            .append("获取WIFI连接下的ip地址").append(getWifiIP(context)).append("\n")
            .append("蓝牙地址").append(getBluetoothMAC(context)).append("\n")
            .append("编译厂商").append(getBuildBrand()).append("\n")
            .append("编译服务器主机").append(getBuildHost()).append("\n")
            .append("描述Build的标签").append(getBuildTags()).append("\n")
            .append("系统编译时间").append(getBuildTime()).append("\n")
            .append("系统编译作者").append(getBuildUser()).append("\n")
            .append("编译系统版本(5.1)").append(getBuildVersionRelease()).append("\n")
            .append("开发代号").append(getBuildVersionCodename()).append("\n")
            .append("源码控制版本号").append(getBuildVersionIncremental()).append("\n")
            .append("编译的SDK").append(getBuildVersionSDK()).append("\n")
            .append("修订版本列表(LMY47D)").append(getBuildID()).append("\n")
            .append("CPU指令集").append(getSupportedABIS()).append("\n")
            .append("硬件制造厂商").append(getManufacturer()).append("\n")
            .append("系统启动程序版本号").append(getBootloader()).append("\n")
            .append("系统版本号").append(getDisplayVersion()).append("\n")
            .append("硬件信息").append(getHardware()).append("\n")
            .append("产品信息").append(getProduct()).append("\n")
            .append("设备信息").append(getDevice()).append("\n")
            .append("主板信息").append(getBoard()).append("\n")
            .append("获取基带版本(无线电固件版本 Api14以上)").append(getRadioVersion()).append("\n")
            .append("获取的浏览器指纹(User-Agent)").append(getUA(context)).append("\n")
            .append("屏幕密度").append(getDensity(context)).append("\n");
        return sb.toString();
    }
	...
	public static String getSysSdk() {
	    ...
	    ALogUtils.i("AppSysMgr-->>getSysLanguage", sdkVersion);
	    return sdkVersion;
	}
	...
	public static String getSysLanguage() {
		...
		ALogUtils.i("AppSysMgr-->>getSysLanguage",  language);
		return language;
	}
	...
	public static String getSysModel() {
		...
		ALogUtils.i("AppSysMgr-->>getSysModel",  model);
		return model;
	}
	...
	public static String getSysRelease() {
		...
		ALogUtils.i("AppSysMgr-->>getSysRelease",  release);
		return release;
	}
	...
	public static String getSysSIMSerialNum(Context content) {
		...
		ALogUtils.i("AppSysMgr-->>getSysSIMSerialNum",  simSerialNumber);
    ...
    public static String getSysCPUSerialNum() {
        ...
        try {
            ...
            e.printStackTrace();
            ALogUtils.e("AppSysMgr-->>getSysCPUSerialNum",  e.getMessage().toString());
        }
        	return cpuSerialNum;
    }
	...
	private static TelephonyManager getSysTelephonyManager(Context content) {
		...
		ALogUtils.i("AppSysMgr-->>getSysTelephonyManager",  telephonyManager + "");
	...
	public static String getSysTelephoneSerialNum(Context content) {
		...
		ALogUtils.i("AppSysMgr-->>getSysTelephoneSerialNum",  deviceId + "");
		return deviceId;
	}
	...
	public static String getSysCarrier(Context content) {
		...
		if (imsi != null && imsi.length() > 0) {
			...
			if (imsi.startsWith("46000") || imsi.startsWith("46002")) {
				//中国移动
				moblieType = "China Mobile 中国移动";
			} else if (imsi.startsWith("46001")) {
				//中国联通
				moblieType = "China Unicom 中国联通";
			} else if (imsi.startsWith("46003")) {
				//中国电信
				moblieType = "China Telecom 中国电信";
			}
		}
		ALogUtils.i("AppSysMgr-->>getSysCarrier",  moblieType);
		return moblieType;
	}
	...
	public static Integer getSysPhoneState(Context context) {
		...
		ALogUtils.i("AppSysMgr-->>getSysPhoneState",  callState + "");
		return callState;
	}
	...
	public static CellLocation getSysPhoneLoaction(Context context) {
		...
		ALogUtils.i("AppSysMgr-->>getSysPhoneLoaction",  cellLocation + "");
		return cellLocation;
	}
	...
	public static String getSysDeviceSoftVersion(Context context) {
		...
		ALogUtils.i("AppSysMgr-->>getSysDeviceSoftVersion",  deviceSoftwareVersion + "");
	...
	public static String getSysPhoneNumber(Context context) {
		...
		ALogUtils.i("AppSysMgr-->>getSysPhoneNumber",  phoneNumber + "");
		return phoneNumber;
	}
	...
	public static String getSysSimCode(Context context) {
		...
		ALogUtils.i("AppSysMgr-->>getSysSimCode",  code + "");
		return code;
	}
	...
	public static String getSysSimPrivatorName(Context context) {
		...
		ALogUtils.i("AppSysMgr-->>getSysSimPrivatorName",  simOperatorName);
	...
	public static String getSysUserPhoneId(Context context) {
		...
		ALogUtils.i("AppSysMgr-->>getSysUserPhoneId",  subscriberId);
		return subscriberId;
	}
	...
	public static DisplayMetrics getSysDisplayMetrics(Activity activity) {
		...
		ALogUtils.i("AppSysMgr-->>getSysDisplayMetrics",  "获取屏幕管理对象为：" + displayMetrics);
	...
	public static int[] getScreenDispaly(Context context) {
		...
		ALogUtils.i("AppSysMgr-->>getScreenDispaly-->>width",  "获取屏幕宽度为：" + width);
		ALogUtils.i("AppSysMgr-->>getScreenDispaly-->>height",  "获取屏幕高度为：" + height);
    ...
    public static Integer getSysScreenWidth(Context context) {
        ...
        ALogUtils.i("AppSysMgr-->>getSysScreenWidth",  "获取屏幕宽度为：" + displayMetrics.widthPixels);
    ...
    public static Integer getSysScreenHeight(Context context)  {
        ...
        DisplayMetrics displayMetrics = new DisplayMetrics();
        wm.getDefaultDisplay().getMetrics(displayMetrics);
        ALogUtils.i("AppSysMgr-->>getSysScreenHeight",  "获取屏幕高度为：" + displayMetrics.heightPixels);
    ...
    public static Integer getSysScreenStatusHeight(Context context)  {
        ...
        try {
            ...
            ALogUtils.i("AppSysMgr-->>getSysScreenStatusHeight",  "获取屏幕状态栏高度为：" + statusHeight);
        } catch (Exception e) {
            e.printStackTrace();
	...
	public static Integer getSysDefaultThreadPoolSize() {
		...
		ALogUtils.i("AppSysMgr-->>getSysDefaultThreadPoolSize",  availableProcessors + "");
    ...
    public static Integer getSysSampleSize(BitmapFactory.Options options, int minSideLength, int maxNumOfPixels) {
        ...
        ALogUtils.i("AppSysMgr-->>getSysSampleSize",  roundedSize + "");
        return roundedSize;
    }
    ...
    public static String getSysLocalIpAddress() {
        ...
        try {
        	...
        	e.printStackTrace();
        	ALogUtils.e("AppSysMgr-->>getSysLocalIpAddress",  e.getMessage().toString());
        }
        	ALogUtils.i("AppSysMgr-->>getSysLocalIpAddress",  hostAddress);
        	return hostAddress;
    }
	...
	public static String getIP(Context ctx) {
		...
		return wifiManager.isWifiEnabled() ? getWifiIP(ctx) : getGPRSIP();
	}
